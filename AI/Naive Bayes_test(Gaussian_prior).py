...
# 데이터 불러오기
data = pd.read_csv('C:\Projects\data_test2.csv')
...
# cv.fit_transform(train_scaled)
# 가우시안 나이브베이즈 객체 생성
nb = GaussianNB(priors = [92/100, 8/100])

# 학습
nb_fitted = nb.fit(train_scaled, train_target)
y_pred = nb_fitted.predict(test_scaled)
...
# 테스트 
print(nb_fitted.predict_proba(test_scaled)[[1, 7, 12]])
print("----------------------------------------------------------------")
print(f"[Test #1] 예측값 : {nb_fitted.predict(test_scaled)[1]}")
print(f"[Test #2] 예측값 : {nb_fitted.predict(test_scaled)[7]}")
print(f"[Test #3] 예측값 : {nb_fitted.predict(test_scaled)[12]}")
print("----------------------------------------------------------------")
print(f"[Test #1] 실제값 : {test_target[1]}")
print(f"[Test #2] 실제값 : {test_target[7]}")
print(f"[Test #3] 실제값 : {test_target[12]}")
